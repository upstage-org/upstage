...
class SaveScene(graphene.Mutation):
    ...
    def mutate(self, info, stage_id, payload, preview, name=None):
        ...
        with ScopedSession() as local_db_session:
            scene = SceneModel(
                owner_id=current_user_id, stage_id=stage_id, payload=payload, scene_preview=preview)
            scene_order = local_db_session.query(SceneModel).filter(
            ...
            if scene:
                ...
                if not user.role in (ADMIN, SUPER_ADMIN):
                    if not user.id == scene.owner_id:
                        return DeleteScene(success=False, message="Only scene owner or admin can delete this scene!")
