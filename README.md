...
- **Upstage Service**: backend code written in Python, Flask, Graphene, SQLAlchemy. This service provide the neccessary API Endpoint, mainly in GraphQL to manage stages/media/players and configurations such as foyer, permissions and notifications.
...
- **MQTT Broker**: this is the magic behinds that make UpStage's performances work. The broker has many topics to be subscribed, such as `meta/demo/chat` can be used to deliveries chat messages between players and audiences in Demo Stage at the `meta` instance. The same broker can also be used for multiple instance of UpStage.
...
- **Event Archive Service**: since MQTT doesn't save all the messages that ever been sent along it, we need to build a service that listening to every message and store it to our Postgres database. You can run an instance of UpStage without event archive running, but all stage changes will be lost after a browser reload, and no replays would be saved.
- **Studio**: just another frontend code written in Vue 3, but use a more modern technology stack, including Vite, TypeScript, TailwindCSS, Antd and Apollo client. Studio focuses on a better media upload/edit workflow, provide a better UI/UX than the version that was created in `Dashboard`.
- **Nginx**: we use Nginx to serve uploaded media and frontend compiled source code as static files. It is also used to pass incoming requests to UpStage service.
...
The compiled source code will be stored in `dist` folder. Later we will setup `Nginx` to serve these content on our root path.
...
yarn build
```
Studio will be serverd at `/studio` path of our instance.
...
UpStage was designed to have multiple instances of it working independently. Each instance could have it own configurations set to get worked.
...
```python
# MongoDB connection. Event Archive won't be able to put messages in queue without this configuration setuped properly, which can lead to inconsistent stage behavior 
MONGO_HOST = ""
MONGO_PORT = 27018
MONGO_DB = "upstage"
...
```python
# You can get these key from running these python files below. Your users won't be able to log in if you change these key after created them, so don't touch them after generated!
CIPHER_KEY='' # Paste the result from fernet_crypto.py
SECRET_KEY='' # Paste the result from running __init__.py
```

```python
# When setuping Streaming Service, a secret key is recommend so that we can setup password protection and prevent your streaming server from being used by strangers. You will need to paste that key here so that we can generate QR codes with correct stream sign, only then the players will be able to broadcast.
STREAM_KEY=''
```

2. `Dashboard` configurations

When building UpStage's frontend code, Vue will look for `.env` file to load configurations. A `.env` file should contain these configurations (all the key must start with VUE_APP otherwise it won't be imported):
...
Studio use a smaller set of configurations compared to `Dashboard`. There is a small different: configuration name in Studio must be started with `VITE_APP`:
