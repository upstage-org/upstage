...
from core.payment.payment import (
    ACCEPT_TYPE,
    cancel_subscription,
    create_customer,
    create_payment_card,
...
class CreateSubscription(graphene.Mutation):
    ...
    def mutate(self, info, email, type, card):
        ...
        if type == "card":
            ...
            subscription = create_subscription(payment_method.id, customer.id, price.id)

            return CreateSubscription(
                success=True, customer_id=customer.id, subscription_id=subscription.id
            )
...
app.add_url_rule(
    f"/{URL_PREFIX}payment_graphql/",
    view_func=GraphQLView.as_view(
        "payment_graphql", schema=payment_schema, graphiql=True
    ),
)
