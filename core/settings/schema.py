...
from core.settings.models import Config as ConfigModel
from core.project_globals import DBSession, ScopedSession, app
from config import NGINX_CONFIG_FILE, URL_PREFIX
    ...
    def mutate(self, info, url):
        with ScopedSession() as local_db_session:
            config = (
                ...
                .filter(ConfigModel.name == TERMS_OF_SERVICE)
                .first()
            )
            if config:
                config.value = url
...
class SaveConfig(graphene.Mutation):
    ...
    class Arguments:
        name = graphene.String(required=True, description="The name of the config.")
        value = graphene.String(required=True, description="The value of the config.")
