...
from config import MQTT_TRANSPORT
        ...
        if client_messages[client_id] == 10:
            ...
            try:
                ...
                if not receive_stat.first():
                    ...
                    receive_stat.update(
                        {
                            ReceiveStat.mqtt_timestamp: datetime.utcnow(),
                            ReceiveStat.payload: payload,
                        },
...
def get_not_alive_users():
    ...
    not_alive_clients = (
        DBSession.query(ReceiveStat)
        .filter(ReceiveStat.mqtt_timestamp < two_minute_ago)
        .all()
    )
