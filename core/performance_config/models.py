...
from core.project_globals import Base, metadata, DBSession, db
from core.user.models import User
from core.asset.models import Asset, Stage
...
class ParentStage(Base, db.Model):
    ...
    child_asset = relationship(
        Asset, foreign_keys=[child_asset_id], back_populates="stages"
    )
...
class PerformanceMQTTConfig(Base, db.Model):
    ...
    performance_config_id = Column(
        Integer, ForeignKey(PerformanceConfig.id), nullable=False, default=0
    )
    owner = relationship(User, foreign_keys=[owner_id])
    performance_config = relationship(
        PerformanceConfig, foreign_keys=[performance_config_id]
    )
