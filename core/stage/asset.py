...
from core.asset.models import Asset as AssetModel, AssetType as AssetTypeModel, MediaTag
        ...
        with ScopedSession() as local_db_session:
            ...
            asset = (
                ...
                .options(
                    ...
                    joinedload(AssetModel.owner),
                    joinedload(AssetModel.asset_license),
                )
...
class UpdateMedia(graphene.Mutation):
    ...
    asset = graphene.Field(lambda: Asset, description="Media updated by this mutation.")
            ...
            if uploaded_frames:
                ...
                for frame in uploaded_frames:
                    ...
                    if not os.path.exists(mediaDirectory):
                        os.makedirs(mediaDirectory)
                    with open(
                        os.path.join(mediaDirectory, unique_filename), "wb"
                    ) as fh:
        ...
        with ScopedSession() as local_db_session:
            ...
            if asset:
                ...
                local_db_session.query(ParentStage).filter(
                    ParentStage.child_asset_id == id
                ).delete(synchronize_session=False)
                local_db_session.query(MediaTag).filter(MediaTag.asset_id == id).delete(
            ...
            if os.path.exists(physical_path):
                ...
                return DeleteMedia(
                    success=True,
                    message="Media deleted successfully but file not existed on storage!",
                )
...
class AssignStages(graphene.Mutation):
    ...
    asset = graphene.Field(lambda: Asset, description="Asset with assigned stages")
