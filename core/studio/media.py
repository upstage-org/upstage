...
from core.asset.models import Asset as AssetModel, MediaTag, Stage as StageModel, Tag
from core.asset.models import AssetType as AssetTypeModel
...
from core.user.models import GUEST, PLAYER, User as UserModel
from core.project_globals import DBSession, ScopedSession, appdir
from config import STREAM_EXPIRY_DAYS, STREAM_KEY
from flask_jwt_extended import get_jwt_identity, jwt_required
from flask_jwt_extended.view_decorators import verify_jwt_in_request
from graphene_sqlalchemy import SQLAlchemyObjectType
from graphene_sqlalchemy.fields import SQLAlchemyConnectionField
from graphql_relay.node.node import from_global_id
from core.licenses.models import AssetLicense, AssetUsage as AssetUsageModel
from core.performance_config.models import ParentStage
...
class Asset(SQLAlchemyObjectType):
    ...
    permissions = graphene.List(
        AssetUsage,
        description="Users who had been granted or acknowledged to use this media",
    )
    privilege = Previlege(description="Permission of the logged in user for this media")
    ...
    def resolve_stages(self, info):
        return [
            {"id": x.stage_id, "name": x.stage.name, "url": x.stage.file_location}
            for x in self.stages.all()
        ]
    ...
    def resolve_privilege(self, info):
        ...
        usage = (
            ...
            .filter(AssetUsageModel.asset_id == self.id)
...
class AssetConnectionField(SQLAlchemyConnectionField):
    ...
    def get_query(cls, model, info, sort=None, **args):
        query = super(AssetConnectionField, cls).get_query(model, info, sort, **args)
        ...
        for field, value in args.items():
            if field == "id":
                ...
                if len(value):
                    query = query.filter(
                        getattr(model, "asset_type").has(AssetTypeModel.name.in_(value))
                    )
            elif field == "owners":
                if len(value):
                    query = query.filter(
                ...
                if len(value):
                    query = (
                        query.join(MediaTag, AssetModel.tags)
...
class UploadFile(graphene.Mutation):
    ...
    class Arguments:
        ...
        filename = graphene.String(required=True, description="Original file name")
...
class AvatarVoice:
    ...
    pitch = graphene.Int(required=True, description="Voice pitch, range from 0 to 100")
    speed = graphene.Int(required=True, description="Voice speed, range from 0 to 350")
...
class SaveMediaInput(graphene.InputObjectType):
    ...
    name = graphene.String(required=True, description="Name of the media")
    urls = graphene.List(graphene.String, description="Uploaded url of files")
    media_type = graphene.String(
        description="Avatar/prop/backdrop,... default to just a generic media",
        default_value="media",
    )
    id = graphene.ID(description="ID of the media (for updating)")
    copyright_level = graphene.Int(description="Copyright level")
    owner = graphene.String(
        description="Owner of the media, in case of uploading for someone else"
    )
    ...
    note = graphene.String(
        description="It would be useful to have a 'Notes' field on the permissions tab, where i could note the actual copyright owner when it's not me, and other information such as what the image is of, when or where taken, this kind of thing."
    )
...
class SaveMedia(graphene.Mutation):
    ...
    asset = graphene.Field(lambda: Asset, description="Media saved by this mutation.")
        ...
        with ScopedSession() as local_db_session:
            ...
            if asset:
                ...
                if file_location:
                    ...
                    if (
                        asset.id
                        and file_location != asset.file_location
                        and "/" not in file_location
                    ):
                        existedAsset = (
                            local_db_session.query(AssetModel)
                ...
                if owner:
                    ...
                    if new_owner:
                        if new_owner.id != asset.owner_id and user.role in (
                            ADMIN,
                            SUPER_ADMIN,
                        ):
            ...
            if tags:
                ...
                for tag in tags:
                    tag_model = (
                        local_db_session.query(Tag).filter(Tag.name == tag).first()
                    )
                    if not tag_model:
