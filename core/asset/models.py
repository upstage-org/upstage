# -*- coding: iso8859-15 -*-
from core.project_globals import db, Base, metadata, app, api, DBSession
from core.user.models import User
from sqlalchemy.orm import relationship
from sqlalchemy.ext.hybrid import hybrid_property
from sqlalchemy import Column, DateTime, String, BigInteger, Integer, ForeignKey, Text
from datetime import datetime
from core.licenses.models import AssetLicense
import os
import sys
...
class Stage(Base, db.Model):
    ...
    attributes = relationship(
        lambda: StageAttribute, lazy="dynamic", back_populates="stage"
    )
    ...
    def cover(self):
        return (
            self.attributes.filter(StageAttribute.name == "cover").first().description
        )

    @hybrid_property
    def visibility(self):
        return (
            self.attributes.filter(StageAttribute.name == "visibility")
            .first()
            .description
        )
    ...
    def status(self):
        return (
            self.attributes.filter(StageAttribute.name == "status").first().description
        )
