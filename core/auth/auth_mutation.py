    ...
    async def mutate(self, info, username_or_email):
        with ScopedSession() as local_db_session:
            if "@" in username_or_email:
                user = (
                    local_db_session.query(User)
                    .filter(User.email == username_or_email)
                    .first()
                )
            else:
                user = (
...
class VerifyPasswordResetMutation(graphene.Mutation):
    ...
    def mutate(self, info, username, otp):
        with ScopedSession() as local_db_session:
            user = (
                local_db_session.query(User).filter(User.username == username).first()
            )

            if not user:
                raise Exception(f"We cannot find any user with this username!")
            ...
            if not otp_record or otp_record.code != otp:
                raise Exception(f"Invalid OTP code. Please try again.")
