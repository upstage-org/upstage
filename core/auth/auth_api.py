...
from config import (
    ...
    GOOGLE_TOKEN_VERIFY,
    FACEBOOK_ACCESS_TOKEN_CREATE,
    FACEBOOK_TOKEN_VERIFY,
    APPLE_APP_ID,
    APPLE_APP_SECRET,
    APPLE_ACCESS_TOKEN_CREATE,
    APPLE_TOKEN_VERIFY,
    APPLE_TEAM_ID,
    VERSION_STRING_IOS,
...
from core.utils.formatting import to_dict

from core.user.models import User, ROLES, PLAYER, GUEST, ADMIN, SUPER_ADMIN
...
from core.auth.models import (
    ...
    new_security_code,
    SIGNUP_VALIDATION,
    SIGNUP_VALIDATION_MISSING_1ST_CODE,
    PUBLIC_LOGIN,
)

from core.user.totp import verify_user_totp
...
class TokenNoList(object):
    def check(self, header, token):
        ...
        if (
            DBSession.query(JWTNoList).filter(JWTNoList.token == token["jti"]).count()
            > 0
        ):
            return True
    ...
    def remove(self):
        with ScopedSession() as local_db_session:
            local_db_session.query(JWTNoList).filter(
                JWTNoList.remove_after < datetime.utcnow()
            ).delete()
...
def call_login_logout(app_entry=True, num_value=None):
    ...
    signin_token = request.json.get(
        "signin_token", None
    )  # This is an auth code for Apple sign-ins.
    if (not username or len(username) <= 0 or len(username) > 100) and not profile_id:
        return make_response(
            jsonify({"error": "Missing/invalid username or email (12)"}), 400
        )
    if (not password or len(password) <= 0 or len(password) > 100) and not profile_id:
        return make_response(
            jsonify({"error": "Missing/invalid password parameter (13)"}), 400
        )
    ...
    if username and password:
        ...
        if "@" in username:
            ...
            if not email or len(email) <= 0 or len(email) > 100:
                return make_response(
                    jsonify({"error": "Invalid username or email (14)"}), 403
                )
        ...
        if user:
            try:
                if not decrypt(user.password) == password:
                    return make_response(
                        jsonify({"error": "Incorrect username or password (16)"}), 403
                    )
            except:
                return make_response(
                    jsonify(
            ...
            if not user:
                return make_response(
                    jsonify(
                        {
                            "error": "User not found. Please verify that you have an Upstage account or contact us."
                        }
                    ),
                    403,
                )
            ...
            if not decrypt(user.password) == password:
                return make_response(
                    jsonify({"error": "Bad email or password (17)"}), 403
                )
            else:
                return make_response(
                    jsonify(
    ...
    if profile_id:
        with ScopedSession() as local_db_session:
            if from_other == "apple":
                ...
                other_profile = (
                    local_db_session.query(AppleProfile)
                    .filter(AppleProfile.id == profile_id)
                    .one()
                )
                ...
                refresh_token = (
                    None
                    if not other_profile.other_profile_json
                    else json.loads(other_profile.other_profile_json)["refresh_token"]
                )
                did_refresh = False
                if refresh_token:
                    # Refresh tokens expire in 24 hours.
                    payload = {
                        "client_id": APPLE_APP_ID,
                        "client_secret": apple_get_client_secret(),
                        "code": signin_token,
                ...
                result = requests.post(
                    APPLE_ACCESS_TOKEN_CREATE, headers=headers, data=payload
                )
                ...
                if not result.ok:
                    ...
                    if did_refresh:
                        ...
                        if not result.ok:
                            ...
                            return make_response(
                                jsonify({"error": "Invalid Source Sign-in (2)."}), 403
                            )

                        did_refresh = False
                    else:
                        return make_response(
                            jsonify({"error": "Invalid Source Sign-in."}), 403
                        )
                ...
                if not did_refresh:
                    ...
                    other_profile.other_profile_json = json.dumps(
                        {"refresh_token": refresh_token}
                    )

                if (
                    "data" in response
                    and "expires_at" in response["data"]
                    and response["data"]["expires_at"]
                ):
                    timeout_minutes = int(
                        (
                            int(response["data"]["expires_at"])
                    ...
                    "data" in response
                    and "expires_in" in response["data"]
                    and response["data"]["expires_in"]
                ):
                ...
                if user and (not other_profile.user_id):
                    ...
                    user = (
                        DBSession.query(User)
                        .filter(User.id == other_profile.user_id)
                        .first()
                    )
                    if (not user) or (not user.active):
                        ...
                        if user:
                            ...
                            existing_code = get_security_code(
                                user.id, SIGNUP_VALIDATION
                            )
                            if not existing_code:
                                existing_code = new_security_code(
                                    user.id,
                                    SIGNUP_VALIDATION_MISSING_1ST_CODE,
                                    pending_a_id=profile_id,
                                    numeric=True,
                                )
                                """
                                raise LostCodeError(
                            ...
                            if not send_acct_verification_code(
                                user.phone, existing_code
                            ):  # always sms
                                app.logger.warning(
                                    f"Tried to send this user a verification code but their phone number is invalid: user_id {user.id} phone {user.phone} "
                                )
                            ...
                            return make_response(
                                jsonify(
                                    {
                                        "error": "User not found. Please verify that you have an Upstage account by sign-in in directly, then retry this new sign-in method. If this does not work please contact us."
                                    }
                                ),
                                403,
                            )
                    ...
                    if other_profile.user_id != user.id:
                        return make_response(
                            jsonify({"error": "Invalid Source Match"}), 403
                        )

            elif from_other == "facebook":
                other_profile = (
                    local_db_session.query(FacebookProfile)
                    .filter(FacebookProfile.id == profile_id)
                    .one()
                )
                ...
                access_token = response["access_token"]
                result = requests.get(
                    FACEBOOK_TOKEN_VERIFY.format(signin_token, access_token)
                )
                ...
                other_profile = (
                    local_db_session.query(GoogleProfile)
                    .filter(GoogleProfile.id == profile_id)
                    .one()
                )
    ...
    if user.role in (SUPER_ADMIN,):
        ...
        app.config["JWT_REFRESH_TOKEN_EXPIRES"] = (
            ...
            if timeout_minutes
            else timedelta(days=JWT_REFRESH_TOKEN_DAYS)
        )
        print(
            f"User id {user.id} token timeout set to either timeout_minutes if not zero: {timeout_minutes}, or this number of days: {JWT_REFRESH_TOKEN_DAYS}"
        )
...
def refresh():
    ...
    if not user:
        ...
        return make_response(
            jsonify({"error": "Your session expired. Please log in again."}), 403
        )
...
def g_fb_a_lookup():
    ...
    if google_id:
        ...
        profile = (
            DBSession.query(GoogleProfile)
            .filter(GoogleProfile.google_id == google_id)
            .first()
        )
        if profile:
            found = True
    ...
    if facebook_id:
        ...
        profile = (
            DBSession.query(FacebookProfile)
    ...
    if apple_id:
        ...
        profile = (
            DBSession.query(AppleProfile)
            .filter(AppleProfile.apple_id == apple_id)
            .first()
        )
        if profile:
            found = True
    ...
    return make_response(
        jsonify({"found": found, "both": both, "profile_id": profile_id}), 200
    )
...
def jwt_required(fn):
    ...
    def wrapper(*args, **kwargs):
        if CHECK_VERSION_STRING:
            ...
            if (
                (version not in (VERSION_STRING_IOS, VERSION_STRING_ANDROID))
                and (ENV_TYPE == "Production")
                and ("/vue_admin/" not in request.referrer)
            ):
                return make_response(
                    jsonify(
                        {
                            "error": "Please update this app for a better user experience."
                        }
                    ),
                    422,
                )
        try:
            ...
            return make_response(
                jsonify({"error": "You have been logged out on this session."}), 403
            )
