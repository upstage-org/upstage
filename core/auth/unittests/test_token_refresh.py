...
def test_token_refresh():
    ...
    return_value = s.post(
        URI + "/auth/access_token/", data=json.dumps(login_payload), headers=headers
    )
    if not return_value.ok:
        raise Exception(
            "Test failed: status %d, response: %s"
            % (return_value.status_code, return_value.text)
        )
    ...
    while True:
        ...
        return_value = s.post(
            URI + "/auth/access_token/refresh/",
            headers=headers,
            data=json.dumps({"key": key}),
        )
        if not return_value.ok:
            raise Exception(
                "Failed to refresh token, response: %s %s"
                % (return_value.status_code, return_value.text)
            )
        result = json.loads(return_value.text)
        headers["X-Access-Token"] = "{0}".format(result["access_token"])
