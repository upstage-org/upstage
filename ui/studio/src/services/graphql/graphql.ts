...
export const createClient = (namespace: string) => ({
    request: async (...params) => {
        ...
        const client = new GraphQLClient(`${configs.GRAPHQL_ENDPOINT}${namespace}/`, options)
        ...
        try {
            ...
            if (!isRefresh && refreshToken && error.response.errors[0].message === 'Signature has expired') {
