...
def reacts_to_anything(f):
    ...
    if f not in _anything_list: _anything_list.append(f)
    return f
    ...
    def inner(f):
        ...
        for r in rules:
            try:
                path, regex = r.split("=")
                re.compile(regex)
                if None in [path, regex]: raise ValueError
    ...
    for f, rules in _payload_map.items():
        ...
        for r in rules:
            try:
                ...
                if not regex.match(val): raise ValueError
            except:
                should_add = False
                break
        if should_add: output.append(f)
    return output
