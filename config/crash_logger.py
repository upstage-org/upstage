...
def log_handler(app):
    ...
    file_handler = TimedRotatingFileHandler(LOG_FILENAME, \
        when='D', interval=1, backupCount=BACKUP_COUNT)
...
def crash_mailer(app,env_type,hostname):
    ...
    from config.settings import (EMAIL_USE_TLS, EMAIL_HOST, EMAIL_HOST_USER, \
            EMAIL_HOST_PASSWORD, EMAIL_PORT, FROM_EMAIL, ADMINS, DEVS)

    mail_handler = SMTPHandler((EMAIL_HOST,EMAIL_PORT), FROM_EMAIL, DEVS,
        'ENV: {0}, {1}: '.format(env_type,hostname),
        credentials=(EMAIL_HOST_USER,EMAIL_HOST_PASSWORD),
        secure = () if EMAIL_USE_TLS else None, timeout=10.0)
