...
from asset.models import Asset, Stage, StageAttribute, MediaTag
...
with session.no_autoflush:
    ...
    for asset in session.query(Asset).filter(not_(Asset.stages.any())).all():
        ...
        session.query(MediaTag).filter(MediaTag.asset_id == asset.id).delete(synchronize_session=False)
    ...
    for ftype in os.listdir(upload_assets_folder):
        if ('.' not in ftype) and ('..' not in ftype) and os.path.isdir(ftype):
            for media in os.listdir("{}/{}".format(upload_assets_folder, ftype)):
                if not session.query(Asset).filter(Asset.file_location == "{}/{}".format(ftype, media)).first():
                    print("🗑️ Deleting file {}/{}".format(ftype, media))
                    try:
                        pathlib.Path("{}/{}/{}".format(upload_assets_folder, ftype, media)).unlink()
                    except:
                        print("Failed to remove {}/{}/{}".format(upload_assets_folder, ftype, media))
